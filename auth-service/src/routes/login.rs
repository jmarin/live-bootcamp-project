...
async fn handle_2fa(
    ...
    if state
        ...
        .write()
        .await
        .add_code(email.clone(), login_attempt.clone(), two_fa_code.clone())
        .await
        .is_err()
    {
    ...
    let subject = "Two Factor Authentication Code";
    let content = two_fa_code;
    match email_client
        .send_email(email, subject, format!("{:?}", content).as_str())
        .await
    {
        Ok(_) => println!(
    ...
    let two_factor_response = TwoFactorAuthResponse {
        ...
        login_attempt_id: login_attempt.as_ref().to_string(),
    };
    (
        jar,
        Ok((
            StatusCode::PARTIAL_CONTENT,
            Json(LoginResponse::TwoFactorAuth(two_factor_response)),
        )),
    )
}
