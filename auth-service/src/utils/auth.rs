...
pub async fn validate_token(
    ...
    let banned_token_result = banned_token_store.read().await.contains_token(token).await;

    match banned_token_result {
        Ok(is_banned) => {
            if is_banned {
...
mod tests {
    ...
    async fn test_validate_token_with_invalid_token() {
        ...
        let banned_store = Arc::new(RwLock::new(HashsetBannedTokenStore::new()));
