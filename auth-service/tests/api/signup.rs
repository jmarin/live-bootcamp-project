...
async fn should_return_201_if_valid_input() {
    ...
    let input_user = serde_json::json!(
        {
            "email": random_email,
    ...
    let response = app.post_signup(&input_user).await;
...
async fn should_return_400_if_invalid_input() {
    ...
    let app = TestApp::new().await;
    let invalid_inputs = [
        serde_json::json!({"email": "email.com", "password": "password123", "requires2FA": true}),
        serde_json::json!({"email": "email@example.com", "password": "pass", "requires2FA": true}),
        serde_json::json!({"email": "", "password": "password123", "requires2FA": false}),
    ];
...
async fn should_return_409_if_email_already_exists() {
    ...
    let response2 = app.post_signup(&input_user).await;
